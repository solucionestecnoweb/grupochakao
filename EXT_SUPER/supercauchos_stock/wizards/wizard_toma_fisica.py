...
class TomaFisica(models.TransientModel):
    ...
    def generate_xls_report(self):
        ...
        ws1.write(row,col+0,"Código",sub_header_style_c)
        ws1.write(row,col+1,"Descripción",sub_header_style_c)
        ws1.col(col+1).width = int((len('Descripción')+36)*256)
        ws1.write(row,col+2,"Modelo",sub_header_style_c)
        ws1.col(col+2).width = int((len('Modelo')+15)*256)
        ws1.write(row,col+3,"Marca",sub_header_style_c)
        ws1.write(row,col+4,"Lonas",sub_header_style_c)
        ws1.col(col+4).width = int((len('Lonas')+2)*256)

        ws1.write(row,col+5,"Stock Inicial",sub_header_style_c)
        ws1.col(col+5).width = int(len('Stock Inicial')*256)
        ws1.write(row,col+6,"Pedido Cliente",sub_header_style_c)
        ws1.col(col+6).width = int(len('Pedido Cliente')*256)
        ws1.write(row,col+7,"No Despachado",sub_header_style_c)
        ws1.col(col+7).width = int((len('No Despachado')+2)*256)
        ws1.write(row,col+8,"FillerT",sub_header_style_c)
        ws1.col(col+8).width = int(len('FillerT')*256)
        ws1.write(row,col+9,"Stock Final",sub_header_style_c)
        ws1.col(col+9).width = int(len('Stock Final')*256)
        ...
        for product in self._get_products():
            ...
            if product.default_code:
                ws1.write(row,col+0, product.default_code,center)
            else:
                ws1.write(row,col+0, '',center)
            # Description
            if product.description:
                ws1.write(row,col+1, product.description,center)
            else:
                ws1.write(row,col+1, product.name,center)
            # Model
            if product.modelo:
                ws1.write(row,col+2, product.modelo,center)
            else:
                ws1.write(row,col+2, '',center)
            # Brand
            if product.brand_id:
                ws1.write(row,col+3, product.brand_id.name,center)
            else:
                ws1.write(row,col+3, '',center)
            # Tarps
            if product.tarps == 0:
                ws1.write(row,col+4,'N/A',center)
            else :
                ws1.write(row,col+4,product.tarps,center)
            # Initial Stock
            stock_ini = 0
            for item in self._initial_stock(product):
                stock_ini = item.total
            ws1.write(row,col+5,stock_ini,center)
            # Client Order
            total_order = 0
            for item in self._get_orders(product.id).move_ids_without_package:
                total_order = total_order + item.quantity_done
            ws1.write(row,col+6, total_order,center)
            ...
            ws1.write(row,col+7, total_not_dis,center)
            # FillerT
            if total_not_dis == 0:
                ws1.write(row,col+8, round(product.filler, 3),center)
            else:
                ws1.write(row,col+8, round(product.filler * total_not_dis, 3),center)
            # Final Stock
            ws1.write(row,col+9, product.qty_available,center)
